...
def apply_leading_rope(x: mx.array, rotary_dim: int, base: float, offset: int | mx.array) -> mx.array:
    """Rotates the leading ``rotary_dim`` dimensions of ``x``, shaped
    ``(B, n_heads, seq_len, head_dim)``, and passes the rest through."""
    return mx.fast.rope(x, rotary_dim, traditional=False, base=base, scale=1.0, offset=offset)
