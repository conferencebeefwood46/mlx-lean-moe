...
class Qwen3_5Attention:
    ...
    def __call__(self, x: mx.array, cache: KVCache) -> mx.array:
        ...
        q_and_gate = quantized_linear(rows, self.q_proj, quant).reshape(
            seq_len, self.n_heads, 2 * self.head_dim
        )
        q, gate = mx.split(q_and_gate, 2, axis=-1)
        q = mx.fast.rms_norm(q, self.q_norm, eps)

        k = quantized_linear(rows, self.k_proj, quant).reshape(
            seq_len, self.n_kv_heads, self.head_dim
        )
        k = mx.fast.rms_norm(k, self.k_norm, eps)
        v = quantized_linear(rows, self.v_proj, quant).reshape(
            seq_len, self.n_kv_heads, self.head_dim
        )
        ...
        q = apply_leading_rope(
            q, self.config.rotary_dim, self.config.rope_theta, offset
        )
        k = apply_leading_rope(
            k, self.config.rotary_dim, self.config.rope_theta, offset
        )
